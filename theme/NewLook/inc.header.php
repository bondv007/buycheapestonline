<div id="header">
	<div class="inner">
		<ul id="nav">
			<?php
				$css = ( intval ( $static_page ) == 0 ? "class='active'" : "" ) ;
			?>
			<li <?php echo $css ?> ><a href="<?php echo base_url ?>">Home</a></li>
			<?php
				if ( ! empty ( $static_pages ) )
					foreach ( $static_pages as $page )
					{
						...
						{
							$url = get_sef_url ( $page["PageManagerID"] , "StaticPage" )."/" ;
							$css = ( strchr($_SERVER['REQUEST_URI'],$url ) ? "class='active'" : "" ) ;
							$url = base_url.$url ;
			?>
				<li <?php echo $css ?>><a href="<?php echo $url ; ?>"><?php echo $page["PageName"] ?></a></li>
			<?php
						}
					}
			?>
