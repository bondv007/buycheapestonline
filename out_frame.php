<?php
	...
	$coupon = $data->select ( "Coupon" , "*" , array ( "CouponID" => intval ( $coupon_id ) ) ) ;
	$coupon = $coupon[0] ;
	if ( ! empty ( $coupon ) )
	{
...
<body>
      ...
      <div id="bar">
        <div id="first">
            ...
            <a class="lnk" href="<? echo(base_url.$web['WebsiteName'].'/'); ?>"> &raquo; more coupons for this store</a>
        </div>    
        <div id="coupon">
          <span>Enter this code at checkout:</span> <strong id="code"><? echo($coupon['CouponCode']); ?></strong> <br /><br />
          <div class="com">This coupon is copied, just paste it at checkout</div>
        </div>
        <div id="vot">
            <form class="voting" action="<? echo(base_url);?>voting_post/" method="post" id="frm_voting_<? echo($coupon['CouponID']); ?>">
                <span>Did the coupon worked?</span>
                <button class="yesVote" name="vote_code" type="submit" value="1" ></button>
                <button class="noVote" name="vote_code" type="submit" value="0" ></button>
                <input name="CouponID" value="<? echo($coupon['CouponID']); ?>" type="hidden">
            </form>
        </div>
        <div class="social">
...
<!-- AddThis Button END -->
        ...
        <a id="close" href="<? echo(base_url); ?>load_frame/<? echo($coupon['CouponID'].'/'); ?>" target="_top">Close</a>
      </div>
      <iframe id="iframe" src="<? echo(base_url); ?>load_frame/<? echo($coupon['CouponID'].'/'); ?>"></iframe>
